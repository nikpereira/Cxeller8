...
const db = createClient({
  ...
  authToken: 'eyJhbGciOiJFZERTQSIsInR5cCI6IkpXVCJ9.eyJhIjoicnciLCJpYXQiOjE3ODA0MDQ0MzQsImlkIjoiMDE5ZTVjOWMtYTIwMS03OTE4LWFjYmUtZjVmMWMwYzRlZGI2IiwicmlkIjoiOGI1YzU5NWItNTU4MS00MGY3LThjNzAtMWZhY2IxNjc5YjlmIn0._XkcWcPa1BhE0bUBiJe7cP_N0_lzhTC-RYP9ezMJ9BBa7So2nPkjE2-ksStVKuWTVO6mEqwcSYCbXqlkkUgNCg',
});
...
async function initDB() {
  ...
  const migrations = [
    ...
    `ALTER TABLE call_notes ADD COLUMN call_date TEXT`,
  ];
  for (const sql of migrations) {
    try { await db.execute(sql); } catch(e) { /* already exists — safe */ }
  }
  ...
  try {
    ...
    if (existing.rows.length === 0) {
      ...
      await db.execute({
        sql: 'INSERT INTO agents (id, name, email, password_hash, role, created_at, auto_record) VALUES (?,?,?,?,?,?,0)',
        args: [uuidv4(), 'Supervisor', 'supervisor@cxeller8.com', hash, 'supervisor', new Date().toISOString()]
      });
      console.log('[DB] Default supervisor created: supervisor@cxeller8.com / supervisor123');
    }
  } catch(e) { console.warn('[DB] Supervisor seed skipped:', e.message); }
}
...
async function saveRecording(rec) {
  const { v4: uuidv4 } = require('uuid');
  await db.execute({
    sql: `INSERT INTO recordings (id, call_id, agent_id, agent_name, customer_name, filename, cloud_url, duration_seconds, recorded_at)
          VALUES (?,?,?,?,?,?,?,?,?)`,
    args: [uuidv4(), rec.callId||'', rec.agentId||'', rec.agentName||'', rec.customerName||'',
           rec.filename, rec.url, rec.duration||0, new Date().toISOString()]
  });
}

async function getRecordings(agentId, limit) {
  const r = await db.execute({
    sql: `SELECT * FROM recordings ${agentId ? 'WHERE agent_id=?' : ''} ORDER BY recorded_at DESC LIMIT ?`,
    args: agentId ? [agentId, limit||50] : [limit||100]
  });
  return r.rows;
}
