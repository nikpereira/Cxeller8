...
function authMiddleware(req, res, next) {
  ...
  if (token) {
    ...
    catch { req.user = { id: 'supervisor', name: 'Supervisor', role: 'supervisor' }; }
  } else {
    req.user = { id: 'supervisor', name: 'Supervisor', role: 'supervisor' };
  }
  next();
}
function supervisorOnly(req, res, next) {
  // Auth temporarily disabled — allow all
  if (!req.user) req.user = { id: 'supervisor', name: 'Supervisor', role: 'supervisor' };
  next();
}
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    // If DB is down, allow supervisor in with hardcoded fallback
    let agent;
    try {
      agent = await DB.findAgentByEmail(email);
    } catch(dbErr) {
      console.error('[Login] DB error:', dbErr.message);
      // Fallback: allow supervisor login without DB
      if (email === 'supervisor@cxeller8.com' && password === 'supervisor123') {
        const token = jwt.sign({ id: 'supervisor', name: 'Supervisor', email, role: 'supervisor' }, JWT_SECRET, { expiresIn: '12h' });
        return res.json({ token, user: { id: 'supervisor', name: 'Supervisor', email, role: 'supervisor' } });
      }
...
app.get('/api/health/groq', async (req, res) => {
  try {
    const RAG = require('./rag');
    const reply = await RAG.answer([], 'Say OK in exactly one word');
...
app.get('/api/health/turso', async (req, res) => {
  try {
    const r = await DB.db.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name");
...
// ── Recordings API ───────────────────────────────────────────────────────────
app.post('/api/recordings', authMiddleware, async (req, res) => {
  try {
    const { filename, url, callId, customerName, agentName, duration } = req.body;
    if (!url || !filename) return res.status(400).json({ error: 'url and filename required' });
    await DB.saveRecording({
      callId, agentId: req.user.id, agentName: agentName || req.user.name,
      customerName, filename, url, duration
    });
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/recordings', authMiddleware, async (req, res) => {
  try {
    ...
    const agentId = req.user.role === 'supervisor' ? null : req.user.id;
    res.json(await DB.getRecordings(agentId));
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// Start server immediately — never block on DB
server.listen(PORT, () => {
  console.log('\n CXeller8 running on port ' + PORT);
});

// Init DB in background — non-fatal
DB.initDB()
  .then(() => console.log('[DB] Turso connected OK'))
  .catch(e => console.error('[DB] Init failed (server still running):', e.message));
